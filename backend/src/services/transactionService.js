...
/**
 * Core transaction creation used by both manual and auto flows.
 * Returns { createdCount, created } exactly like your controller.
 */
export async function createTransactionForUser({ userId, body }) {
  const {
    accountId,
    categoryId,
    type,
  ...
  if (type === "investment") {
    ...
    if (requiresSymbolUnits) {
      if (!sym) throw new Error("assetSymbol is required for stock/crypto");
  ...
  // 2) Optional future copy (same behavior as your controller)
  if (nextDate) {
    ...
    if (!exists) {
      ...
      try {
        ...
        // ignore extra-copy failure to match your controller behavior
      }
    }
  }
